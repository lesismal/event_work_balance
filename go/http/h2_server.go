...
func (sc *h2ServerConn) fail(err *H2ConnError) {
	...
	sc.sendLocked(h2AppendGoAway(nil, sc.maxClientID, err.Code, err.Reason))
	sc.closed = true
	sc.goAway = true
	clear(sc.streams)
	sc.mu.Unlock()
...
func (sc *h2ServerConn) handleHeaderBlock(id uint32, block []byte, endStream bool) error {
	...
	case sc.closed || sc.goAway || sc.peerGoingAway:
		sc.mu.Unlock()
		return nil
	case uint32(len(sc.streams)) >= sc.maxStreams:
		sc.mu.Unlock()
...
func (sc *h2ServerConn) closeFinishedLocked() {
	if sc.closed || !sc.peerGoingAway || len(sc.streams) > 0 {
		return
	}
	sc.closed = true
	sc.conn.CloseAfterSend()
...
func (st *h2ServerStream) respond(req *stdhttp.Request, response Response) error {
	...
	if status < 200 || status > 999 {
		// Interim responses are not supported, as in HTTP/1.
	...
	for key, values := range response.Header {
		if key == "" || textproto.CanonicalMIMEHeaderKey(key) == "" || !h2ValidHeaderName(strings.ToLower(key)) {
			return errors.New("http: invalid response header name")
		}
		for _, value := range values {
			if !validHeaderValue(value) {
				return errors.New("http: invalid response header value")
			}
		}
	}
	...
	for key, values := range response.Header {
		...
		for _, value := range values {
			block = sc.enc.AppendField(block, name, value, name == "set-cookie")
		}
	}
